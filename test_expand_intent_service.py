...
SERVICE_URL = "http://99.1.82.207:8080/llm-studio/v1/api/task/generate/syncapi/busan_ai_llm/service_category"
...
def parse_service_answer(answer: str) -> Dict[str, Any]:
    """
    서비스 분류 응답(answer)에서 service 객체를 추출.
    의도 파싱과 동일한 방식으로, 'service' 키 블록을 찾습니다.
    """
    try:
        ...
        if not match:
            return {"error": "no_json_block_matched"}
        json_string = match.group(0)
        parsed = ServiceInfo.model_validate_json(json_string).model_dump()
        return parsed
    except ValidationError as ve:
        return {"error": f"validation_error: {ve}"}
    except Exception as e:
...
def main() -> None:
    ...
    with httpx.Client() as client:
        ...
        for i, original_query in enumerate(queries, start=1):
            ...
            # -------------------------
            # 5) 서비스 분류 요청 (의도분석과 동일한 바디)
            # -------------------------
            service_payload = {"user_query": query_complete}
            service_call = robust_post(client, SERVICE_URL, service_payload)

            if not (
                service_call.get("status_code")
                and 200 <= service_call["status_code"] < 300
            ):
                results_failed.append(
                    {
                        "stage": "service_api",
            ...
            # 6) 서비스 분류 결과 파싱
            service_body = service_call["response"] or {}
            service_llm_result = (service_body or {}).get("llm_result", {})
            service_answer = service_llm_result.get("answer")
            if not isinstance(service_answer, str):
                results_failed.append(
                    {
                        "stage": "service_parse",
                        "reason": "answer_not_string",
            ...
            parsed_service = parse_service_answer(service_answer)
            if "error" in parsed_service or not parsed_service.get("service"):
                results_failed.append(
                    {
                        "stage": "service_parse",
                        "reason": parsed_service.get("error") or "missing_service",
                        "original_user_query": original_query,
                        "query_complete": query_complete,
                        "expand_request": expand_payload,
                        "expand_result": expand_call,
            ...
            # 모든 단계 성공 → results.jsonl 저장용 레코드
            results_ok.append(
                {
                    "original_user_query": original_query,
                    "expand": {
                    ...
                    "parsed_service": parsed_service,  # {"service": {"no": ..., "service_name": ...}}
                }
            )
